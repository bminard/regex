...
void perl_matcher<BidiIterator, Allocator, traits>::estimate_max_state_count(std::random_access_iterator_tag*)
{
   ...
   if(dist == 0)
      dist = 1;
   std::ptrdiff_t states = re.size(); // where is the guarantee that re.size() <= (std::numeric_limits<std::ptrdiff_t>::max)()?
   if(states == 0)
      states = 1;
   else if(states > (std::numeric_limits<std::ptrdiff_t>::max)() / states)
   {
